...
public:
    ...
    */
	...
	virtual int setLidarPose(const std::string &lidar_name,int pose) const
	{
		...
		for (uint i = 0; i < count_lidars; i++)
		{
			LidarBase* current_lidar = /*static_cast<LidarBase*>*/(LidarBase*)(getSensors().getByType(SensorBase::SensorType::Lidar, i));
			if (current_lidar != nullptr && (current_lidar->getName() == lidar_name || lidar_name == ""))
			{
				lidar = current_lidar;
				break;
